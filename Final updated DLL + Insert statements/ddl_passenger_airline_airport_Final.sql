...
currency_key int references currency(currency_key) ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
nation_key int not null REFERENCES nation(nation_key) ON DELETE NO ACTION,
logistics_factility int not null REFERENCES logistics(logistics_id) ON DELETE NO ACTION,
airport_code varchar(3) unique not null,
address_key integer not null  REFERENCES address (address_key) ON DELETE NO ACTION
)
;
----NATION----
...
nation_key int references nation(nation_key) ON DELETE NO ACTION,
date_of_birth date
)
...
catering_id int not null references catering(catering_id) ON DELETE NO ACTION,
tower_id int references tower(tower_id) ON DELETE NO ACTION,
...
customer_key int references customer(customer_key) ON DELETE NO ACTION,
flight_key int references flight(flight_key) ON DELETE CASCADE,
...
customer_key int references customer(customer_key) ON DELETE CASCADE,
luggage_key int references luggage(luggage_key) ON DELETE CASCADE,
luggage_date timestamp not null unique,
PRIMARY KEY(customer_key, luggage_key, luggage_date)
);
...
flight_key  int references flight(flight_key) ON DELETE CASCADE,
customer_key  int references customer(customer_key) ON DELETE CASCADE,
indirectbookig_date timestamp not null,
price numeric not null,
CHECK (price >= 0)
);
...
price numeric not null,
currency_key int references currency(currency_key) ON DELETE NO ACTION,
flight_segment_key int references flight_segment(flight_segment_key) ON DELETE NO ACTION,
ordered_food varchar(255),
class_of_seat varchar(255),
customer_key int references customer(customer_key) ON DELETE CASCADE,
luggage_key int references luggage(luggage_key) ON DELETE NO ACTION,
gate_number char(10) not null,
...
(
  ...
  CONSTRAINT airc_fk FOREIGN KEY (airline_key)
      REFERENCES airline_company (airline_key) 
      ON UPDATE NO ACTION ON DELETE NO ACTION,
...
(
  ...
  CONSTRAINT airplane_type_fk2 FOREIGN KEY (admission_doc_key)
      REFERENCES admission_doc (admission_doc_key)
      ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
(
  ...
  CONSTRAINT empl_fk FOREIGN KEY (personnel_key)
      REFERENCES empl_on_board (personnel_key)
      ON UPDATE NO ACTION ON DELETE CASCADE
);
...
(
  ...
  CONSTRAINT segment_fk FOREIGN KEY (flight_segment_key)
      REFERENCES flight_segment (flight_segment_key)
      ON UPDATE NO ACTION ON DELETE CASCADE,
  CONSTRAINT type_fk FOREIGN KEY (type_key)
      REFERENCES airplane_type (type_key)
      ON UPDATE NO ACTION ON DELETE CASCADE
...
(
  ...
  CONSTRAINT tissue_fk FOREIGN KEY (tissue_key)
      REFERENCES technical_issue (tissue_key)
      ON UPDATE NO ACTION ON DELETE CASCADE
);
...
CONSTRAINT airport_logistic_pk PRIMARY KEY (logistics_id,airport_key), 
...
tenant_id integer not null references tenant(tenant_id) ON DELETE NO ACTION,
...
runway_id  int references runway(runway_id) ON DELETE NO ACTION,
airport_key  int references airport(airport_key) ON DELETE CASCADE
);
...
runway_id  int references runway(runway_id) ON DELETE NO ACTION,
flight_contoller_id  int references flight_controller(flight_controller_id) ON DELETE NO ACTION,
flight_key  int references flight(flight_key) ON DELETE NO ACTION,
start_airport int references airport(airport_key) ON DELETE NO ACTION,
end_airport int references airport(airport_key) ON DELETE NO ACTION,
state_date timestamp,
end_date timestamp,
managed_by int references airport(airport_key) ON DELETE NO ACTION
);
